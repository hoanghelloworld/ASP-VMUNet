...
class PatchMerging2D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = self.reduction(x)   # B H/2 W/2 C*2
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y1, y2, y3, y4 = self.forward_core(x)   # [B, D, H, W]
...
class VSSLayer(nn.Module):
    ...
    def forward(self, x):
        for blk in self.blocks:         # 先进行mamba的运算
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.num_features = dims[-1]        # dims里面的最后一个维度 768
        self.dims = dims

        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=in_chans, embed_dim=self.embed_dim,
            norm_layer=norm_layer if patch_norm else None)  # patch size传入为4，默认按照四个进行一次embeded
        ...
        self.pos_drop = nn.Dropout(p=drop_rate) # dropout比率
        # 采用随机深度 drp rate
    ...
    def forward_features(self, x): # x [1, 3, 256, 256]
        skip_list = []
        x = self.patch_embed(x) # x [1, 64, 64, 96]  , dims=[96, 192, 384, 768]     H/4
        if self.ape: # absolute position embedding vit
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x) # 这里的drop_rate = 0
        # 返回最下面的 x feature 和 中间的四步的 feature
        for layer in self.layers: 
            skip_list.append(x) # x [1, 96, 64, 64]         # 注意这里！是先把mamba输出的添加到list里面，然后在进行下采样
